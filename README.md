...
| [`RunThunder/`](./RunThunder/) | 走るブラックサンダーがメニューバーで動く **macOS メニューバーアプリ**（RunCat 風）。システムダッシュボード・Claude 使用量の本数表示に加え、チョコでバッテリー残量を表示（battery を統合）。→ [README](./RunThunder/README.md) |
| [`blackthunder-battery/`](./blackthunder-battery/) | バッテリー残量を板チョコで表示する **macOS メニューバー ガジェット**。ネイティブ表示は [RunThunder](./RunThunder/) に統合済み。単体 Swift アプリ / SwiftBar プラグイン版を残しています。→ [README](./blackthunder-battery/README.md) |
