...
- **🔋 チョコでバッテリーを表示**: 走るキャラとは別に、**2 つ目のメニューバーアイコン**
  としてブラックサンダーの板チョコでバッテリー残量を表示（残量ぶんだけチョコが残り、
  充電中は稲妻）。走るキャラはコマ送り、こちらは残量に応じて独立更新します。設定で
  ON/OFF 可（既定 ON、バッテリー非搭載機では自動で隠す）。旧 `blackthunder-battery` の
  ネイティブアプリを統合したものです。
